...
import { z } from 'zod';
import { create_invoice, load_invoice_data, update_invoice } from '@/redux/Action/actions';
import { toast } from 'sonner';
import { Switch } from '@/components/ui/switch';
import AddEditFormSkeletonLoader from './AddEditFormSkeletonLoader';

const addEditInvoiceSchema = z.object({
  shop_name: z
    .string()
...
type AddEditInvoiceValues = z.infer<typeof addEditInvoiceSchema>
...
export default function AddEditInvoice() {
  ...
  const form = useForm<AddEditInvoiceValues>({
    resolver: zodResolver(addEditInvoiceSchema),
    defaultValues: {
      shop_name: "",
      shop_owner_name: "",
      shop_email: "",
  ...
  useEffect(() => {
    ...
    const fetchShopData = async () => {
      try {
        setIsLoading(true);
        const response = await load_invoice_data(dispatch, invoiceId);
  ...
  const onSubmit = async (data: AddEditInvoiceValues) => {
    ...
    try {
      ...
      let response;
      if (invoiceId) {
        response = await update_invoice(invoiceId, payload);
      } else {
        response = await create_invoice(payload);
      }
  ...
  return (
    <Card className="border-none shadow-none bg-transparent">
      <CardHeader className='gap-0.5 px-2'>
        <CardTitle className='text-lg font-semibold'>{invoiceId ? 'Edit Invoice' : 'Add New Invoice'}</CardTitle>
        <CardDescription className='text-sm text-muted-foreground'>
          {invoiceId ? 'Edit the Invoice details' : 'Add a new invoice to your account'}
        </CardDescription>
      </CardHeader>
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    ...
                    {invoiceId ? 'Saving...' : 'Adding shop...'}
                  </>
                ) : (
                  invoiceId ? 'Save changes' : 'Add shop'
                )}
              </Button>
            </div>
          </form>
